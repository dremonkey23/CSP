...
def render_markdown(scored: list[ScoredCandidate], limit: int = 10) -> str:
    ...
    for i, s in enumerate(good, 1):
        ...
        lines += [
            ...
            f'   Breakeven: ${s.breakeven:.2f} | Assignment discount: {fmt_pct(s.assignment_discount)} | Day: {fmt_pct(c.day_change_pct) if c.day_change_pct is not None else "—"} | RSI14: {c.rsi_14 if c.rsi_14 is not None else "—"} | Delta: {c.delta} | OI: {c.open_interest} | Vol: {c.volume} | Spread: {s.spread_pct:.1f}%',
            ''
        ]
