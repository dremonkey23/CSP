...
def make_static_html(source: str) -> str:
    ...
    html = html.replace(
        ...
        "      const urls = [\n"
        "        { source: 'live branch', url: RAW_PAGES_ROOT + path + '?v=' + stamp },\n"
        "        { source: 'dashboard', url: path + '?v=' + stamp },\n"
        "      ];\n"
        "      const attempts = await Promise.all(urls.map(async candidate => {\n"
        "        try {\n"
        "          const res = await fetch(candidate.url, { cache: 'no-store' });\n"
        "          if (!res.ok) throw new Error('HTTP ' + res.status);\n"
        "          return { ...candidate, data: await res.json() };\n"
        "        } catch (error) {\n"
        "          return { ...candidate, error };\n"
        "        }\n"
        "      }));\n"
        "      const valid = attempts.filter(candidate => candidate.data);\n"
        "      if (!valid.length) {\n"
        "        throw attempts[0].error || attempts[1].error || new Error('Failed loading dashboard data');\n"
        "      }\n"
        "      const timestamp = candidate => {\n"
        "        const data = candidate.data;\n"
        "        return String(Array.isArray(data) ? (data[0]?.updated_at || '') : (data.updated_at || ''));\n"
        "      };\n"
        "      valid.sort((a, b) => timestamp(b).localeCompare(timestamp(a)));\n"
        "      const selected = valid[0];\n"
        "      dataSource = selected.source;\n"
        "      return { ok: true, json: async () => selected.data };\n"
        "    }\n\n"
        "    async function loadReports() {\n",
    )
